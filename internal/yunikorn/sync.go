...
func (s *Service) syncQueues(ctx context.Context, partitions []*model.Partition) ([]*model.Queue, error) {
	logger := log.FromContext(ctx)

	allQueues := make([]*model.Queue, 0)
	for _, p := range partitions {
		logger.Info("syncing queues for partition", "partition", p.Name)
		queues, err := s.syncPartitionQueues(ctx, p)
		if err != nil {
			logger.Errorf("could not sync queues for partition %s: %v", p.Name, err)
			continue
		}
...
func (s *Service) syncPartitionQueues(ctx context.Context, partition *model.Partition) ([]*model.Queue, error) {
	logger := log.FromContext(ctx)
	clientQueues, err := s.client.GetPartitionQueues(ctx, partition.Name)
	if err != nil {
		return nil, fmt.Errorf("could not get queues for partition %s: %v", partition.Name, err)
	}
	...
	now := time.Now().UnixNano()
	allQueues := make([]*model.Queue, 0, len(queues))
	for _, q := range queues {
		...
		if !ok || current.DeletedAt != nil { // either not exists or deleted
			...
			if err := s.repo.InsertQueue(ctx, queue); err != nil {
				logger.Errorf("could not insert queue %s: %v", q.QueueName, err)
			}
			continue
		}

		current.MergeFrom(q)
		allQueues = append(allQueues, current)
		if err := s.repo.UpdateQueue(ctx, current); err != nil {
			logger.Errorf("could not update queue %s: %v", q.QueueName, err)
		}
	}
	...
	for _, q := range lookup {
		...
		if err := s.repo.UpdateQueue(ctx, q); err != nil {
			logger.Errorf("failed to update deleted at for queue %q: %v", q.QueueName, err)
		}
	}
...
func (s *Service) syncApplications(ctx context.Context) error {
	...
	now := time.Now().UnixNano()
	allApplications := make([]*model.Application, 0, len(applications))
	for _, a := range applications {
		...
		current.MergeFrom(a)
		allApplications = append(allApplications, current)

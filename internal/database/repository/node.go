...
(id,  created_at, deleted_at, cluster_id, partition, nodes_util_list)
VALUES
(@id, @created_at, @deleted_at, @cluster_id, @partition, @nodes_util_list)
`
...
func (s *PostgresRepository) GetNodesUtils(
	...
	for rows.Next() {
		...
		err := rows.Scan(&nu.ID, &nu.CreatedAt, &nu.DeletedAt, &nu.ClusterID, &nu.Partition, &nu.NodesUtilList)
		if err != nil {
